...
async def suggest_code_fix(
    ...
    """
    Format agent-generated code fix as GitHub suggestion markdown.

    === CONTEXT ===
    Purpose: Format code fixes (generated by the agent) as GitHub suggestion syntax
    ...
    - new_code: The corrected code (generated by the agent)
    - issue_category: Type of issue ("naming", "type_hint", "security", "bug", etc.)
    - file_path: Optional file path for language detection (for RAG citation)
    ...
    3. EXTRACT citation from style guide context
    4. FORMAT as GitHub suggestion markdown with citation
    5. RETURN formatted suggestion

    Edge Cases:
    - RAG unavailable: Format without citation
    - Empty new_code: Return error message

    === INTEGRATION ===
    Triggered by: Agent tool call when agent has generated a fix
    Calls into: RAG service (for citation only)
    ...
    Args:
        ...
        new_code: The corrected code (agent has already generated this)
